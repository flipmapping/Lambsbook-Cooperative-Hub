...
import { registrationConfirmationTemplate } from "./emailTemplates";
...
const email = registrationConfirmationTemplate({
  ...
  await recordProspectActivity(
    ...
    "registration_confirmation_email_sent",
    {
      ...
      subject: "Thank you for registering your interest in CTBC University",
      provider: "resend",
      channel: "email",
    },
  ).catch((err) =>
    console.error(
      "[ProspectActivity] Failed to record email activity:",
      err,
    ),
  );
