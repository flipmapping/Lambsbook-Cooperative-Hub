...
export interface NotificationPayload {
  ...
  message: string;
  enquiryId?: string;
  /** Optional: if present, a Zalo notification is sent in addition to email. */
  zaloPhone?: string;
}
...
export async function sendNotification(payload: NotificationPayload): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // Zalo: additive side-channel — fires only when zaloPhone is present.
    // Does not alter the email result or the notification record.
...
async function sendZaloCopy(
  ...
  try {
    ...
    if (json.error !== 0) {
      console.error(
        `[Zalo] API error ${json.error}: ${json.message}`
      );
      return {
        success: false,
        error: `Zalo API error ${json.error}: ${json.message}`,
      };
    }
