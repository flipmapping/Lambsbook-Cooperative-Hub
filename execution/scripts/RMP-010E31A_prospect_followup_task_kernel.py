#!/usr/bin/env python3
"""
RMP-010E31A — Prospect Follow-up Task Kernel
CIB Authority: RMP-010E31A / Derived From: FDR-010E31
Infrastructure Prerequisite: INF-010E31A — CERTIFIED
  Verified infrastructure: growth.prospect_followup_tasks

Implements create, update, list, and complete operations for prospect
follow-up tasks using the certified growth.prospect_followup_tasks table.

Minimum bounded mutation corridor (4 server files):
  MODIFY  server/lib/supabase-types.ts
  MODIFY  server/lib/supabase-dal.ts
  MODIFY  server/services/admissions.ts
  MODIFY  server/routes.ts

No client files. No schema changes. No adjacent surface modifications.

Anchors (verified unique from post-RMP-010E30A Repository Truth):
  FA:  ProspectActivityInsert terminal interface
  FB1: DAL import block (ProspectActivity, ProspectActivityInsert)
  FB2: corridor: listActivities return → getMemberByUserId
  FC:  terminal service block (getProspectActivities)
  FD1: routes admissions import line
  FD2: GET /activities close → countries GET corridor

Quality gate: EXEC-STD-001 + EXEC-STD-002 + EOS 2.0
"""

import sys
from pathlib import Path

# ── ANSI ──────────────────────────────────────────────────────
GREEN  = "\033[92m"
RED    = "\033[91m"
YELLOW = "\033[93m"
CYAN   = "\033[96m"
RESET  = "\033[0m"
BOLD   = "\033[1m"

_step_results: dict[str, str] = {}

def _ok(msg: str)   -> None: print(f"  {GREEN}✓{RESET}  {msg}")
def _info(msg: str) -> None: print(f"  {CYAN}→{RESET}  {msg}")
def _head(msg: str) -> None: print(f"\n{BOLD}{msg}{RESET}")

def abort(reason: str) -> None:
    banner = (
        f"\n{'=' * 42}\n"
        f"RMP-010E31A\n"
        f"FAIL\n\n"
        f"{reason}\n\n"
        f"Repository structure differs from established\n"
        f"Repository Truth.\n\n"
        f"Mutation aborted. No files modified.\n"
        f"{'=' * 42}\n"
    )
    print(f"{RED}{banner}{RESET}")
    sys.exit(1)

def blocked(reason: str) -> None:
    banner = (
        f"\n{'=' * 42}\n"
        f"RMP-010E31A\n"
        f"BLOCKED\n\n"
        f"{reason}\n\n"
        f"{'=' * 42}\n"
    )
    print(f"{RED}{banner}{RESET}")
    sys.exit(2)

# ── Repository constants ───────────────────────────────────────
REPO_ROOT_ANCHORS = ["package.json", "client", "server", "web"]

FILE_TYPES   = Path("server/lib/supabase-types.ts")
FILE_DAL     = Path("server/lib/supabase-dal.ts")
FILE_SERVICE = Path("server/services/admissions.ts")
FILE_ROUTES  = Path("server/routes.ts")

# ── Idempotency markers ────────────────────────────────────────
IDEM_TYPES_TASK     = "export interface FollowupTask {"
IDEM_DAL_CREATE     = "async createFollowupTask("
IDEM_DAL_LIST       = "async listFollowupTasks("
IDEM_DAL_UPDATE     = "async updateFollowupTask("
IDEM_DAL_COMPLETE   = "async completeFollowupTask("
IDEM_SVC_CREATE     = "export async function createProspectFollowupTask("
IDEM_SVC_LIST       = "export async function getProspectFollowupTasks("
IDEM_ROUTE_GET      = 'app.get("/api/admissions/prospects/:id/followup-tasks"'
IDEM_ROUTE_POST     = 'app.post("/api/admissions/prospects/:id/followup-tasks"'

# ════════════════════════════════════════════════════════════════
# FILE A — supabase-types.ts
# Append FollowupTask + FollowupTaskInsert + FollowupTaskUpdate after
# ProspectActivityInsert (verified terminal interface, count: 1).
# ════════════════════════════════════════════════════════════════

_FA_SEARCH = (
    "export interface ProspectActivityInsert {\n"
    "  prospect_id: string;\n"
    "  activity_type: string;\n"
    "  metadata?: Record<string, unknown> | null;\n"
    "}"
)
_FA_REPLACE = (
    "export interface ProspectActivityInsert {\n"
    "  prospect_id: string;\n"
    "  activity_type: string;\n"
    "  metadata?: Record<string, unknown> | null;\n"
    "}\n"
    "\n"
    "export interface FollowupTask {\n"
    "  id: string;\n"
    "  prospect_id: string;\n"
    "  title: string;\n"
    "  description: string | null;\n"
    "  due_date: string | null;\n"
    "  completed: boolean;\n"
    "  completed_at: string | null;\n"
    "  created_at: string;\n"
    "}\n"
    "\n"
    "export interface FollowupTaskInsert {\n"
    "  prospect_id: string;\n"
    "  title: string;\n"
    "  description?: string | null;\n"
    "  due_date?: string | null;\n"
    "}\n"
    "\n"
    "export interface FollowupTaskUpdate {\n"
    "  title?: string;\n"
    "  description?: string | null;\n"
    "  due_date?: string | null;\n"
    "}"
)
_FA_LABEL = "Append FollowupTask, FollowupTaskInsert, FollowupTaskUpdate to supabase-types.ts"

FILE_A_OPS = [(_FA_SEARCH, _FA_REPLACE, _FA_LABEL)]

# ════════════════════════════════════════════════════════════════
# FILE B — supabase-dal.ts
# Op B1: extend import block with followup task types
# Op B2: insert createFollowupTask, listFollowupTasks, updateFollowupTask,
#         completeFollowupTask in corridor:
#         listActivities return → getMemberByUserId
# ════════════════════════════════════════════════════════════════

_FB1_SEARCH = (
    "  ProspectActivity, ProspectActivityInsert,\n"
    "  TutorStatus\n"
    "} from './supabase-types';"
)
_FB1_REPLACE = (
    "  ProspectActivity, ProspectActivityInsert,\n"
    "  FollowupTask, FollowupTaskInsert, FollowupTaskUpdate,\n"
    "  TutorStatus\n"
    "} from './supabase-types';"
)
_FB1_LABEL = "Add FollowupTask, FollowupTaskInsert, FollowupTaskUpdate to DAL imports"

_FB2_SEARCH = (
    "    return data ?? [];\n"
    "  }\n"
    "\n"
    "  async getMemberByUserId"
)
_FB2_REPLACE = (
    "    return data ?? [];\n"
    "  }\n"
    "\n"
    "  async createFollowupTask(\n"
    "    data: FollowupTaskInsert,\n"
    "  ): Promise<FollowupTask> {\n"
    "    this.ensureConfigured();\n"
    "    const supabase = getSupabaseAdmin();\n"
    "\n"
    "    const { data: task, error } = await supabase\n"
    "      .schema('growth').from('prospect_followup_tasks')\n"
    "      .insert({\n"
    "        prospect_id:  data.prospect_id,\n"
    "        title:        data.title,\n"
    "        description:  data.description ?? null,\n"
    "        due_date:     data.due_date ?? null,\n"
    "      })\n"
    "      .select()\n"
    "      .single();\n"
    "\n"
    "    if (error) throw new Error(`Failed to create follow-up task: ${error.message}`);\n"
    "    return task;\n"
    "  }\n"
    "\n"
    "  async listFollowupTasks(\n"
    "    prospectId: string,\n"
    "  ): Promise<FollowupTask[]> {\n"
    "    this.ensureConfigured();\n"
    "    const supabase = getSupabaseAdmin();\n"
    "\n"
    "    const { data, error } = await supabase\n"
    "      .schema('growth').from('prospect_followup_tasks')\n"
    "      .select('*')\n"
    "      .eq('prospect_id', prospectId)\n"
    "      .order('created_at', { ascending: true });\n"
    "\n"
    "    if (error) throw new Error(`Failed to list follow-up tasks: ${error.message}`);\n"
    "    return data ?? [];\n"
    "  }\n"
    "\n"
    "  async updateFollowupTask(\n"
    "    taskId: string,\n"
    "    data: FollowupTaskUpdate,\n"
    "  ): Promise<FollowupTask> {\n"
    "    this.ensureConfigured();\n"
    "    const supabase = getSupabaseAdmin();\n"
    "\n"
    "    const { data: task, error } = await supabase\n"
    "      .schema('growth').from('prospect_followup_tasks')\n"
    "      .update({\n"
    "        ...(data.title        !== undefined && { title: data.title }),\n"
    "        ...(data.description  !== undefined && { description: data.description }),\n"
    "        ...(data.due_date     !== undefined && { due_date: data.due_date }),\n"
    "      })\n"
    "      .eq('id', taskId)\n"
    "      .select()\n"
    "      .single();\n"
    "\n"
    "    if (error) throw new Error(`Failed to update follow-up task: ${error.message}`);\n"
    "    return task;\n"
    "  }\n"
    "\n"
    "  async completeFollowupTask(taskId: string): Promise<FollowupTask> {\n"
    "    this.ensureConfigured();\n"
    "    const supabase = getSupabaseAdmin();\n"
    "\n"
    "    const { data: task, error } = await supabase\n"
    "      .schema('growth').from('prospect_followup_tasks')\n"
    "      .update({ completed: true, completed_at: new Date().toISOString() })\n"
    "      .eq('id', taskId)\n"
    "      .select()\n"
    "      .single();\n"
    "\n"
    "    if (error) throw new Error(`Failed to complete follow-up task: ${error.message}`);\n"
    "    return task;\n"
    "  }\n"
    "\n"
    "  async getMemberByUserId"
)
_FB2_LABEL = (
    "Insert createFollowupTask, listFollowupTasks, updateFollowupTask, "
    "completeFollowupTask after listActivities in SupabaseDAL"
)

FILE_B_OPS = [
    (_FB1_SEARCH, _FB1_REPLACE, _FB1_LABEL),
    (_FB2_SEARCH, _FB2_REPLACE, _FB2_LABEL),
]

# ════════════════════════════════════════════════════════════════
# FILE C — server/services/admissions.ts
# Append 4 service functions after the verified terminal block.
# ════════════════════════════════════════════════════════════════

_FC_SEARCH = (
    "export async function getProspectActivities(prospectId: string) {\n"
    "  return supabaseDAL.listActivities(prospectId);\n"
    "}"
)
_FC_REPLACE = (
    "export async function getProspectActivities(prospectId: string) {\n"
    "  return supabaseDAL.listActivities(prospectId);\n"
    "}\n"
    "\n"
    "export async function createProspectFollowupTask(\n"
    "  prospectId: string,\n"
    "  title: string,\n"
    "  description?: string | null,\n"
    "  dueDate?: string | null,\n"
    ") {\n"
    "  return supabaseDAL.createFollowupTask({\n"
    "    prospect_id:  prospectId,\n"
    "    title,\n"
    "    description:  description ?? null,\n"
    "    due_date:     dueDate ?? null,\n"
    "  });\n"
    "}\n"
    "\n"
    "export async function getProspectFollowupTasks(prospectId: string) {\n"
    "  return supabaseDAL.listFollowupTasks(prospectId);\n"
    "}\n"
    "\n"
    "export async function updateProspectFollowupTask(\n"
    "  taskId: string,\n"
    "  title?: string,\n"
    "  description?: string | null,\n"
    "  dueDate?: string | null,\n"
    ") {\n"
    "  return supabaseDAL.updateFollowupTask(taskId, {\n"
    "    ...(title       !== undefined && { title }),\n"
    "    ...(description !== undefined && { description }),\n"
    "    ...(dueDate     !== undefined && { due_date: dueDate }),\n"
    "  });\n"
    "}\n"
    "\n"
    "export async function completeProspectFollowupTask(taskId: string) {\n"
    "  return supabaseDAL.completeFollowupTask(taskId);\n"
    "}"
)
_FC_LABEL = "Append followup task service functions to admissions service"

FILE_C_OPS = [(_FC_SEARCH, _FC_REPLACE, _FC_LABEL)]

# ════════════════════════════════════════════════════════════════
# FILE D — server/routes.ts
# Op D1: extend admissions import with followup task functions
# Op D2: add 4 followup task routes after GET /activities close
# ════════════════════════════════════════════════════════════════

_FD1_SEARCH = (
    'import { submitProspectRegistration, listProspects, getProspect, updateProspectStage,'
    ' recordProspectLifecycleEvent, getProspectLifecycleEvents,'
    ' recordProspectActivity, getProspectActivities }'
    ' from "./services/admissions";'
)
_FD1_REPLACE = (
    'import { submitProspectRegistration, listProspects, getProspect, updateProspectStage,'
    ' recordProspectLifecycleEvent, getProspectLifecycleEvents,'
    ' recordProspectActivity, getProspectActivities,'
    ' createProspectFollowupTask, getProspectFollowupTasks,'
    ' updateProspectFollowupTask, completeProspectFollowupTask }'
    ' from "./services/admissions";'
)
_FD1_LABEL = "Extend admissions import with followup task service functions"

_FD2_SEARCH = (
    '      res.status(500).json({ error: "Failed to list prospect activities" });\n'
    "    }\n"
    "  });\n"
    "\n"
    '  app.get("/api/countries"'
)
_FD2_REPLACE = (
    '      res.status(500).json({ error: "Failed to list prospect activities" });\n'
    "    }\n"
    "  });\n"
    "\n"
    # GET /followup-tasks
    '  app.get("/api/admissions/prospects/:id/followup-tasks",'
    " async (req: Request, res: Response) => {\n"
    "    try {\n"
    "      const tasks = await getProspectFollowupTasks(req.params.id);\n"
    "      res.json(tasks);\n"
    "    } catch (error) {\n"
    '      console.error("List follow-up tasks error:", error);\n'
    '      res.status(500).json({ error: "Failed to list follow-up tasks" });\n'
    "    }\n"
    "  });\n"
    "\n"
    # POST /followup-tasks
    '  app.post("/api/admissions/prospects/:id/followup-tasks",'
    " async (req: Request, res: Response) => {\n"
    "    try {\n"
    "      const { title, description, due_date } = req.body;\n"
    '      if (!title || typeof title !== "string") {\n'
    '        return res.status(400).json({ error: "title is required" });\n'
    "      }\n"
    "      const task = await createProspectFollowupTask(\n"
    "        req.params.id,\n"
    "        title,\n"
    "        description ?? null,\n"
    "        due_date ?? null,\n"
    "      );\n"
    "      res.status(201).json(task);\n"
    "    } catch (error) {\n"
    '      console.error("Create follow-up task error:", error);\n'
    '      res.status(500).json({ error: "Failed to create follow-up task" });\n'
    "    }\n"
    "  });\n"
    "\n"
    # PATCH /followup-tasks/:taskId
    '  app.patch("/api/admissions/prospects/:id/followup-tasks/:taskId",'
    " async (req: Request, res: Response) => {\n"
    "    try {\n"
    "      const { title, description, due_date } = req.body;\n"
    "      const task = await updateProspectFollowupTask(\n"
    "        req.params.taskId,\n"
    "        title,\n"
    "        description,\n"
    "        due_date,\n"
    "      );\n"
    "      res.json(task);\n"
    "    } catch (error) {\n"
    '      console.error("Update follow-up task error:", error);\n'
    '      res.status(500).json({ error: "Failed to update follow-up task" });\n'
    "    }\n"
    "  });\n"
    "\n"
    # POST /followup-tasks/:taskId/complete
    '  app.post("/api/admissions/prospects/:id/followup-tasks/:taskId/complete",'
    " async (req: Request, res: Response) => {\n"
    "    try {\n"
    "      const task = await completeProspectFollowupTask(req.params.taskId);\n"
    "      res.json(task);\n"
    "    } catch (error) {\n"
    '      console.error("Complete follow-up task error:", error);\n'
    '      res.status(500).json({ error: "Failed to complete follow-up task" });\n'
    "    }\n"
    "  });\n"
    "\n"
    '  app.get("/api/countries"'
)
_FD2_LABEL = (
    "Add GET, POST, PATCH, and complete routes for "
    "/api/admissions/prospects/:id/followup-tasks"
)

FILE_D_OPS = [
    (_FD1_SEARCH, _FD1_REPLACE, _FD1_LABEL),
    (_FD2_SEARCH, _FD2_REPLACE, _FD2_LABEL),
]

# ════════════════════════════════════════════════════════════════
# MUTATION ENGINE
# ════════════════════════════════════════════════════════════════

def _apply_one(working: str, search: str, replace: str, label: str) -> str:
    pre = working.count(search)
    if pre == 0:
        abort(
            f"Structural anchor not found: {label}\n\n"
            f"Expected exactly one occurrence of:\n"
            f"{search[:120].strip()}\n\n"
            "Repository structure differs from Repository Truth."
        )
    if pre > 1:
        abort(
            f"Ambiguous structural anchor: {label}\n\n"
            f"Found {pre} occurrences (expected exactly 1) of:\n"
            f"{search[:120].strip()}\n\n"
            "Cannot safely apply bounded replacement."
        )
    result = working.replace(search, replace, 1)
    if result.count(replace) != 1:
        abort(f"Post-replacement count error: {label}")
    return result


def _write_and_verify(path: Path, content: str) -> None:
    path.write_text(content, encoding="utf-8")
    on_disk = path.read_text(encoding="utf-8")
    if on_disk != content:
        abort(f"Round-trip verification failed: {path.name}")
    _ok(f"Round-trip verified ({len(on_disk)} chars on disk)")


def _mutate_file(root: Path, rel_path: Path, ops: list[tuple[str, str, str]]) -> None:
    path = root / rel_path
    _info(f"Reading {rel_path}")
    working = path.read_text(encoding="utf-8")
    _ok(f"Read ({len(working)} chars)")
    for i, (search, replace, label) in enumerate(ops, 1):
        _info(f"  Op {i}/{len(ops)}: {label}")
        _info(f"    Anchor: {search[:60].strip()!r}...")
        working = _apply_one(working, search, replace, label)
        _ok(f"    Op {i} applied")
    _info(f"Writing {rel_path}")
    _write_and_verify(path, working)


# ════════════════════════════════════════════════════════════════
# STAGE 1 — Repository Anchor Verification
# ════════════════════════════════════════════════════════════════

def stage_1_repository(root: Path) -> None:
    _head("STAGE 1 — Repository Anchor Verification")
    for anchor in REPO_ROOT_ANCHORS:
        if (root / anchor).exists():
            _ok(f"Repository anchor present: {anchor}")
        else:
            abort(f"Repository anchor missing: {anchor}")
    _ok("All repository anchors confirmed")
    _step_results["Repository anchors"] = "PASS"


# ════════════════════════════════════════════════════════════════
# STAGE 2 — Infrastructure + Dependency Verification
# ════════════════════════════════════════════════════════════════

def stage_2_verify(root: Path) -> tuple[str, str, str, str]:
    _head("STAGE 2 — Infrastructure and Dependency Verification")

    _info("Infrastructure: INF-010E31A — CERTIFIED per governance package")
    _ok("Infrastructure CERTIFIED: growth.prospect_followup_tasks (SQL applied)")

    for path_obj, label in [
        (root / FILE_TYPES,   FILE_TYPES),
        (root / FILE_DAL,     FILE_DAL),
        (root / FILE_SERVICE, FILE_SERVICE),
        (root / FILE_ROUTES,  FILE_ROUTES),
    ]:
        if not path_obj.exists():
            abort(f"File not found: {label}")

    types_src = (root / FILE_TYPES).read_text(encoding="utf-8")
    dal_src   = (root / FILE_DAL).read_text(encoding="utf-8")
    svc_src   = (root / FILE_SERVICE).read_text(encoding="utf-8")
    rts_src   = (root / FILE_ROUTES).read_text(encoding="utf-8")

    _ok(f"{FILE_TYPES} located ({len(types_src)} chars)")
    _ok(f"{FILE_DAL} located ({len(dal_src)} chars)")
    _ok(f"{FILE_SERVICE} located ({len(svc_src)} chars)")
    _ok(f"{FILE_ROUTES} located ({len(rts_src)} chars)")

    # Dependency verification — RMP-010E30A must be certified
    for src, marker, dep in [
        (types_src, "export interface ProspectActivityInsert {",
         "ProspectActivityInsert — terminal interface anchor (RMP-010E30A VERIFIED)"),
        (types_src, "export interface ProspectActivity {",
         "ProspectActivity — RMP-010E30A dependency VERIFIED"),
        (dal_src,   "async listActivities(",
         "listActivities — corridor anchor (RMP-010E30A VERIFIED)"),
        (dal_src,   "async getMemberByUserId(",
         "getMemberByUserId — corridor boundary VERIFIED"),
        (dal_src,   "  ProspectActivity, ProspectActivityInsert,",
         "DAL import block anchor VERIFIED"),
        (svc_src,   "export async function getProspectActivities(",
         "getProspectActivities — service anchor (RMP-010E30A VERIFIED)"),
        (rts_src,   'app.get("/api/admissions/prospects/:id/activities"',
         "GET /activities — RMP-010E30A VERIFIED"),
        (rts_src,   'app.get("/api/countries"',
         "countries GET — corridor boundary VERIFIED"),
    ]:
        if marker in src:
            _ok(f"Dependency VERIFIED: {dep}")
        else:
            if "RMP-010E30A" in dep:
                blocked(
                    f"Dependency BLOCKED: {dep}\n\n"
                    f"RMP-010E30A must be executed and certified before RMP-010E31A.\n\n"
                    f"Missing: {marker}"
                )
            abort(f"Dependency UNVERIFIED: {dep}\n\nExpected: {marker}")

    # Structural corridor verification on clean path
    if IDEM_DAL_CREATE not in dal_src:
        _info("Verifying structural corridor anchors (clean path)")
        for search, src_file, label in [
            (_FA_SEARCH, types_src, "types terminal interface (ProspectActivityInsert)"),
            (_FB2_SEARCH, dal_src,  "DAL corridor (listActivities return → getMemberByUserId)"),
            (_FC_SEARCH,  svc_src,  "admissions service terminal function"),
            (_FD1_SEARCH, rts_src,  "routes admissions import line"),
            (_FD2_SEARCH, rts_src,  "routes corridor (GET /activities close → countries GET)"),
        ]:
            if search not in src_file:
                abort(f"Structural anchor not found: {label}")
            _ok(f"Structural anchor VERIFIED: {label}")
    else:
        _ok("Structural anchor checks skipped — followup task kernel already present")

    _step_results["Infrastructure + dependency verification"] = "PASS"
    return types_src, dal_src, svc_src, rts_src


# ════════════════════════════════════════════════════════════════
# STAGE 3 — Idempotency Verification
# ════════════════════════════════════════════════════════════════

def stage_3_idempotency(types_src: str, dal_src: str,
                        svc_src: str, rts_src: str) -> bool:
    _head("STAGE 3 — Idempotency Verification")

    checks = {
        "FollowupTask interface in types":           IDEM_TYPES_TASK in types_src,
        "createFollowupTask() in DAL":               IDEM_DAL_CREATE in dal_src,
        "listFollowupTasks() in DAL":                IDEM_DAL_LIST in dal_src,
        "updateFollowupTask() in DAL":               IDEM_DAL_UPDATE in dal_src,
        "completeFollowupTask() in DAL":             IDEM_DAL_COMPLETE in dal_src,
        "createProspectFollowupTask() in service":   IDEM_SVC_CREATE in svc_src,
        "getProspectFollowupTasks() in service":     IDEM_SVC_LIST in svc_src,
        "GET /followup-tasks route":                 IDEM_ROUTE_GET in rts_src,
        "POST /followup-tasks route":                IDEM_ROUTE_POST in rts_src,
    }

    for label, present in checks.items():
        _info(f"{label}  → {present}")

    all_present = all(checks.values())
    any_present = any(checks.values())

    if all_present:
        _ok("Follow-up Task Kernel already applied — no mutation required")
        _step_results["Follow-up Task Kernel"] = "PASS (Already Satisfied)"
        return True

    if any_present:
        present = [k for k, v in checks.items() if v]
        absent  = [k for k, v in checks.items() if not v]
        abort(
            "Partial Follow-up Task Kernel detected.\n\n"
            "Present:\n" + "".join(f"  {p}\n" for p in present) +
            "\nAbsent:\n"  + "".join(f"  {a}\n" for a in absent) +
            "\nManual review required."
        )

    _ok("Clean state confirmed — proceeding with mutation")
    return False


# ════════════════════════════════════════════════════════════════
# STAGE 4 — Repository Mutation
# ════════════════════════════════════════════════════════════════

def stage_4_mutate(root: Path) -> None:
    _head("STAGE 4 — Repository Mutation")

    _head("  File A — server/lib/supabase-types.ts [MODIFY]")
    _mutate_file(root, FILE_TYPES, FILE_A_OPS)

    _head("  File B — server/lib/supabase-dal.ts [MODIFY]")
    _mutate_file(root, FILE_DAL, FILE_B_OPS)

    _head("  File C — server/services/admissions.ts [MODIFY]")
    _mutate_file(root, FILE_SERVICE, FILE_C_OPS)

    _head("  File D — server/routes.ts [MODIFY]")
    _mutate_file(root, FILE_ROUTES, FILE_D_OPS)

    _step_results["Follow-up Task Kernel"] = "PASS"


# ════════════════════════════════════════════════════════════════
# STAGE 5 — Post-Mutation Verification
# ════════════════════════════════════════════════════════════════

def stage_5_post_verify(root: Path, idempotent: bool = False) -> None:
    label_str = "STAGE 5 — Post-Verification (Idempotent Path)" if idempotent \
                else "STAGE 5 — Post-Mutation Verification"
    _head(label_str)

    types_src = (root / FILE_TYPES).read_text(encoding="utf-8")
    dal_src   = (root / FILE_DAL).read_text(encoding="utf-8")
    svc_src   = (root / FILE_SERVICE).read_text(encoding="utf-8")
    rts_src   = (root / FILE_ROUTES).read_text(encoding="utf-8")
    _ok(f"Re-read: types({len(types_src)}), dal({len(dal_src)}), "
        f"service({len(svc_src)}), routes({len(rts_src)}) chars")

    count_checks: list[tuple[str, str, int, str]] = [
        (types_src, "export interface FollowupTask {",         1, "FollowupTask interface"),
        (types_src, "export interface FollowupTaskInsert {",   1, "FollowupTaskInsert interface"),
        (types_src, "export interface FollowupTaskUpdate {",   1, "FollowupTaskUpdate interface"),
        (types_src, "export interface ProspectActivityInsert {", 1, "ProspectActivityInsert preserved"),
        (dal_src,   "async createFollowupTask(",               1, "createFollowupTask() in DAL"),
        (dal_src,   "async listFollowupTasks(",                1, "listFollowupTasks() in DAL"),
        (dal_src,   "async updateFollowupTask(",               1, "updateFollowupTask() in DAL"),
        (dal_src,   "async completeFollowupTask(",             1, "completeFollowupTask() in DAL"),
        (dal_src,   ".schema('growth').from('prospect_followup_tasks')", 4,
         "prospect_followup_tasks table (4 operations)"),
        (svc_src,   "export async function createProspectFollowupTask(", 1,
         "createProspectFollowupTask() in service"),
        (svc_src,   "export async function getProspectFollowupTasks(",   1,
         "getProspectFollowupTasks() in service"),
        (svc_src,   "export async function updateProspectFollowupTask(", 1,
         "updateProspectFollowupTask() in service"),
        (svc_src,   "export async function completeProspectFollowupTask(", 1,
         "completeProspectFollowupTask() in service"),
        (rts_src,   'app.get("/api/admissions/prospects/:id/followup-tasks"',  1, "GET /followup-tasks"),
        (rts_src,   'app.post("/api/admissions/prospects/:id/followup-tasks"', 1, "POST /followup-tasks"),
        (rts_src,   'app.patch("/api/admissions/prospects/:id/followup-tasks/:taskId"', 1,
         "PATCH /followup-tasks/:taskId"),
        (rts_src,   'app.post("/api/admissions/prospects/:id/followup-tasks/:taskId/complete"', 1,
         "POST /followup-tasks/:taskId/complete"),
    ]
    for src, marker, expected, desc in count_checks:
        count = src.count(marker)
        if count == expected:
            _ok(f"Count [{desc}]: {count} (expected {expected})")
        else:
            abort(f"Count verification failed: {desc} — count={count}, expected={expected}")

    # Content checks
    for marker, desc in [
        (".schema('growth').from('prospect_followup_tasks')",  "growth schema used for tasks"),
        ("completed: true, completed_at: new Date().toISOString()", "complete sets completed + timestamp"),
        (".order('created_at', { ascending: true });",         "tasks ordered chronologically"),
        ('res.status(400).json({ error: "title is required"', "POST validates title"),
        ("res.status(201).json(task)",                         "POST returns 201 Created"),
    ]:
        if marker in dal_src or marker in rts_src:
            _ok(f"Content: {desc}")
        else:
            abort(f"Content missing: {desc}")

    # Preservation
    for src, marker, desc in [
        (types_src, "export interface ProspectActivity {",     "ProspectActivity preserved"),
        (dal_src,   "async listActivities(",                   "listActivities preserved"),
        (dal_src,   "async listLifecycleEvents(",              "listLifecycleEvents preserved"),
        (svc_src,   "export async function getProspectActivities(", "getProspectActivities preserved"),
        (rts_src,   'app.get("/api/admissions/prospects/:id/activities"', "GET /activities preserved"),
        (rts_src,   'app.get("/api/admissions/prospects/:id/events"',     "GET /events preserved"),
        (rts_src,   'app.patch("/api/admissions/prospects/:id/stage"',    "PATCH /stage preserved"),
        (rts_src,   'app.post("/api/admissions/prospects"',               "POST prospects preserved"),
    ]:
        if marker in src:
            _ok(f"Preserved: {desc}")
        else:
            abort(f"Preserved content missing: {desc}")

    state = "PASS (Already Satisfied)" if idempotent else "PASS"
    _step_results["Post-verification"] = state


# ════════════════════════════════════════════════════════════════
# STAGE 6 — Runtime Contract Preservation
# ════════════════════════════════════════════════════════════════

def stage_6_e2e(root: Path) -> None:
    _head("STAGE 6 — Runtime Contract Preservation Verification")

    rts_src = (root / FILE_ROUTES).read_text(encoding="utf-8")

    _info("Verifying all admissions runtime contracts preserved")
    for route, desc in [
        ('app.post("/api/admissions/prospects"',                          "POST prospects"),
        ('app.get("/api/admissions/prospects"',                           "GET prospects"),
        ('app.get("/api/admissions/prospects/:id"',                       "GET prospect"),
        ('app.patch("/api/admissions/prospects/:id/stage"',               "PATCH stage"),
        ('app.get("/api/admissions/prospects/:id/events"',                "GET events"),
        ('app.get("/api/admissions/prospects/:id/activities"',            "GET activities"),
        ('app.get("/api/admissions/prospects/:id/followup-tasks"',        "GET followup-tasks"),
        ('app.post("/api/admissions/prospects/:id/followup-tasks"',       "POST followup-tasks"),
        ('app.patch("/api/admissions/prospects/:id/followup-tasks/:taskId"', "PATCH followup-task"),
        ('app.post("/api/admissions/prospects/:id/followup-tasks/:taskId/complete"', "POST complete"),
    ]:
        if route in rts_src:
            _ok("Runtime contract: " + route.split('"')[1])
        else:
            abort(f"Runtime contract missing: {desc}")

    _ok("All runtime contracts verified")
    _step_results["Runtime contract preservation"] = "PASS"


# ════════════════════════════════════════════════════════════════
# SUMMARY
# ════════════════════════════════════════════════════════════════

def print_summary() -> None:
    _head("MUTATION SUMMARY")
    max_len = max(len(k) for k in _step_results) if _step_results else 0
    for step, state in _step_results.items():
        colour = YELLOW if "Already" in state else GREEN
        print(f"  {step.ljust(max_len)}   {colour}{state}{RESET}")

    all_pass = all(s.startswith("PASS") for s in _step_results.values())
    print()
    if all_pass:
        print(f"{BOLD}{GREEN}══ RESULT: PASS ══{RESET}{RESET}")
        print()
        print("  Repository files in final state:")
        print(f"    MODIFY  {FILE_TYPES}")
        print(f"    MODIFY  {FILE_DAL}")
        print(f"    MODIFY  {FILE_SERVICE}")
        print(f"    MODIFY  {FILE_ROUTES}")
        print()
        print("  New API endpoints:")
        print("    GET    /api/admissions/prospects/:id/followup-tasks")
        print("    POST   /api/admissions/prospects/:id/followup-tasks")
        print("    PATCH  /api/admissions/prospects/:id/followup-tasks/:taskId")
        print("    POST   /api/admissions/prospects/:id/followup-tasks/:taskId/complete")
        print()
        print("  Next: RMP-010E31B")
        print()
        print("  Second execution: PASS (Already Satisfied)")
    else:
        print(f"{BOLD}{RED}══ RESULT: FAIL ══{RESET}{RESET}")
        sys.exit(1)


# ════════════════════════════════════════════════════════════════
# ROOT RESOLUTION
# ════════════════════════════════════════════════════════════════

def resolve_root() -> Path:
    script_dir     = Path(__file__).resolve().parent
    candidate_root = script_dir.parent.parent
    cwd_root       = Path.cwd()

    for candidate in [candidate_root, cwd_root]:
        if all((candidate / a).exists() for a in REPO_ROOT_ANCHORS):
            return candidate

    if len(sys.argv) > 1:
        explicit = Path(sys.argv[1]).resolve()
        if all((explicit / a).exists() for a in REPO_ROOT_ANCHORS):
            return explicit

    abort(
        "Repository root not found.\n\n"
        f"Expected anchors: {REPO_ROOT_ANCHORS}\n\n"
        "Pass the repository root as an argument:\n"
        "  python RMP-010E31A_prospect_followup_task_kernel.py /path/to/repo"
    )
    raise SystemExit(1)


# ════════════════════════════════════════════════════════════════
# ENTRY POINT
# ════════════════════════════════════════════════════════════════

def main() -> None:
    print(f"\n{BOLD}RMP-010E31A — Prospect Follow-up Task Kernel{RESET}")
    print(f"{BOLD}CIB Authority: RMP-010E31A / FDR-010E31{RESET}\n")
    print(f"  Prerequisite: INF-010E31A CERTIFIED\n")

    root = resolve_root()
    _info(f"Repository root: {root}")
    _info(f"Minimum bounded mutation corridor:")
    _info(f"  MODIFY  {FILE_TYPES}")
    _info(f"  MODIFY  {FILE_DAL}")
    _info(f"  MODIFY  {FILE_SERVICE}")
    _info(f"  MODIFY  {FILE_ROUTES}")

    stage_1_repository(root)
    types_src, dal_src, svc_src, rts_src = stage_2_verify(root)
    already = stage_3_idempotency(types_src, dal_src, svc_src, rts_src)

    if already:
        stage_5_post_verify(root, idempotent=True)
        stage_6_e2e(root)
    else:
        stage_4_mutate(root)
        stage_5_post_verify(root, idempotent=False)
        stage_6_e2e(root)

    print_summary()


if __name__ == "__main__":
    main()
