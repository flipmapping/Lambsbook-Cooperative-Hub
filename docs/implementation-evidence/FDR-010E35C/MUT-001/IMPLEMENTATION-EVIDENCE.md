...
/**
 * Canonical identity classification for the authenticated runtime user.
 *
 * This type classifies who the authenticated user is.
 * It does not carry runtime data, JWT payload, or database records.
 *
 * Runtime identity resolution is introduced incrementally through
 * controlled bounded mutations under the continuation pipeline.
 */
...
export interface RuntimeState {
  ...
  /**
   * Canonical identity classification for the authenticated runtime user.
   * Optional — populated by the identity resolution stage when activated.
   * Undefined until runtime identity resolution is delegated.
   */
...
/**
 * Stage 4a output — produced by _resolveAuthenticatedIdentity().
 *
 * Carries the identity classification for the authenticated runtime user.
 * The resolved field is undefined until runtime identity resolution
 * is activated in a subsequent bounded mutation.
 *
 * DORMANT — returns undefined. Activation deferred to a subsequent
 * bounded implementation authority.
 */
interface _ResolvedAuthenticatedIdentity {
  readonly resolvedIdentity: AuthenticatedIdentity | undefined;
}
...
/**
 * Stage 4a — Runtime Identity Resolution.
 *
 * Extension point for canonical identity classification of the authenticated
 * runtime user. Accepts the determined runtime state and resolves the
 * AuthenticatedIdentity classification.
 *
 * Position in pipeline:
 *   _determineRuntimeState()
 *        ↓
 *   _resolveAuthenticatedIdentity()   ← this stage
 *        ↓
 *   _prepareRuntimePublication()
 *
 * DORMANT — returns undefined for resolvedIdentity.
 * Actual identity resolution is deferred to a subsequent bounded
 * implementation authority. This stage establishes the architectural
 * boundary only.
 */
function _resolveAuthenticatedIdentity(
  _determined: _DeterminedRuntimeState,
): _ResolvedAuthenticatedIdentity {
  return {
...
function _prepareRuntimePublication(
  _determined: _DeterminedRuntimeState,
  _identity: _ResolvedAuthenticatedIdentity,
  ...
  return {
    publication: {
      ...
      authenticatedIdentity: _identity.resolvedIdentity,
    },
  };
}
...
export async function postAuthenticationContinuation(
  ...
  const resolvedIdentity =
    _resolveAuthenticatedIdentity(runtimeState);

  const prepared =
    _prepareRuntimePublication(runtimeState, resolvedIdentity);
