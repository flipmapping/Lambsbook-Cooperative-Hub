...
export default function HubAuthCallback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        // Get the hash fragment (Supabase puts tokens in hash)
        ...
        if (error || urlError) {
          setStatus("error");
          setErrorMessage(errorDescription || urlErrorDescription || "Authentication failed");
          return;
        }
        ...
        if (accessToken) {
          ...
          // No token found - might be a different auth flow or error
          setStatus("error");
          setErrorMessage("No authentication token received. Please try signing up again.");
        }
      } catch (e) {
      ...
      <Card className="max-w-md w-full">
        ...
        {status === "error" && (
          <CardContent className="flex flex-col gap-2">
            <Button onClick={() => setLocation("/hub/signup")} data-testid="button-try-again">
              Try Again
            </Button>
            <Button variant="outline" onClick={() => setLocation("/hub")} data-testid="button-back-to-hub">
              Back to Hub
            </Button>
          </CardContent>
        )}
