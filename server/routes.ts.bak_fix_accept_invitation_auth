...
export async function registerRoutes(
  ...
  // --------------------------------------------------
  app.get("/api/hub/member/invitations", attachUserContext, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = req.user;
      ...
      console.error("member/invitations route error:", err);
      return res.status(500).json({
        message: "Server error",
        debug: err instanceof Error ? err.stack || err.message : String(err),
      });
    }
  });
  ...
  app.get("/api/hub/member/me", attachUserContext, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = req.user;
      if (!user) {
  ...
  app.get("/api/hub/member/earnings", attachUserContext, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/hub/member/invitees", attachUserContext, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/hub/member/invitor", attachUserContext, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/hub/member/referrals", attachUserContext, requireAuth, async (req: Request, res: Response) => {
    try {
