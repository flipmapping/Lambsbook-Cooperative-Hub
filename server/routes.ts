...
export async function registerRoutes(
  ...
  app.get("/api/debug-auth-user", async (req: Request, res: Response) => {
    try {
      const authHeader = req.headers.authorization;
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.json({ user: null, message: 'No token provided' });
      }
      ...
      if (!resp.ok) {
        return res.json({ user: null, message: 'Token invalid or expired', status: resp.status });
      }
