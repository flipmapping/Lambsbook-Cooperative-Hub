...
// for users in entryState === "invited_pending_acceptance".
//
// ISOLATION RULES:
...
export function InvitationAcceptanceSection({
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        setErrorMessage("Unable to accept the invitation right now. Please try again.");
      }
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-indigo-800/60 bg-indigo-950/40 p-5"
    >
      <div className="mb-4">
        ...
        <p className="mt-2 text-sm text-slate-300">
          You have been invited to join the cooperative. Accepting this invitation
          activates your membership and unlocks full cooperative participation.
        </p>
        <p className="mt-1 text-xs text-slate-500">
          Invitation acceptance is the sole membership activation surface. This
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="rounded-lg border border-indigo-500 bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50"
        >
